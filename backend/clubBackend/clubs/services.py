...
# from sqlalchemy.exc import IntegrityError, SQLAlchemyError
# from .database import get_db_connection, clubs_table
...
#     @staticmethod
#     def get_all_clubs(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
#         stmt = select(clubs_table).order_by(clubs_table.c.created_at.desc()).offset(skip).limit(limit)
#         with get_db_connection() as conn:
...
#         stmt = select(clubs_table).where(clubs_table.c.club_id == club_id)
#         with get_db_connection() as conn:
#             row = conn.execute(stmt).fetchone()
#             return dict(row) if row else None

#     @staticmethod
#     def get_club_by_name(name: str) -> Optional[Dict[str, Any]]:
#         stmt = select(clubs_table).where(clubs_table.c.name == name)
#         with get_db_connection() as conn:
#             row = conn.execute(stmt).fetchone()
#             return dict(row) if row else None
...
#         if ClubService.get_club_by_name(club_data['name']):
#             raise ValueError(f"Club '{club_data['name']}' already exists")

#         stmt = insert(clubs_table).values(
#             name=club_data['name'],
#             description=club_data['description'],
#             created_by=club_data['created_by'],
#             created_at=datetime.now()
#         ).returning(
#             clubs_table.c.club_id,
#             clubs_table.c.name,
#             clubs_table.c.description,
#             clubs_table.c.created_by,
#             clubs_table.c.created_at
#         )
...
#                 raise ValueError(f"User with ID {club_data['created_by']} does not exist")
...
#         stmt = update(clubs_table).where(clubs_table.c.club_id == club_id).values(**club_data).returning(
#             clubs_table.c.club_id,
#             clubs_table.c.name,
#             clubs_table.c.description,
#             clubs_table.c.created_by,
...
#     @staticmethod
#     def delete_club(club_id: int) -> bool:
#         stmt = delete(clubs_table).where(clubs_table.c.club_id == club_id)
#         with get_db_connection() as conn:
#             result = conn.execute(stmt)
#             return result.rowcount > 0

#     @staticmethod
#     def get_clubs_by_creator(user_id: int) -> List[Dict[str, Any]]:
#         stmt = select(clubs_table).where(clubs_table.c.created_by == user_id).order_by(clubs_table.c.created_at.desc())
#         with get_db_connection() as conn:
#             result = conn.execute(stmt).fetchall()
#             return [dict(row) for row in result]

#     @staticmethod
#     def search_clubs(search_term: str) -> List[Dict[str, Any]]:
#         pattern = f"%{search_term}%"
#         stmt = select(clubs_table).where(
#             or_(clubs_table.c.name.ilike(pattern), clubs_table.c.description.ilike(pattern))
#         ).order_by(clubs_table.c.created_at.desc())
#         with get_db_connection() as conn:
#             result = conn.execute(stmt).fetchall()
#             return [dict(row) for row in result]
