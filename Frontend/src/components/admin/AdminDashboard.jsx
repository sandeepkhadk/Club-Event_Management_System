...
import { useAuthContext } from "../../context/provider/AuthContext";
import { useUserRole } from '../../context/hooks/useUserRole';
import { Calendar, Menu, X, Users, PlusCircle, CalendarCheck, Settings, LogOut } from 'lucide-react';
import MemberManagement from './MemberManagement';
import EventCreationForm from './EventCreation';
import EventList from './EventList';
import EditEventModal from './EditEventModal';
...
// ─── Parrot Green palette ───────────────────────────────────────────────────
// Active:   bg-[#39D353]  text-slate-900  shadow-[#39D353]/40
// Hover:    hover:bg-[#2bb545]/20  hover:text-[#39D353]
// ────────────────────────────────────────────────────────────────────────────

const NAV_ITEMS = [
  { key: 'members',        label: 'Member Requests', icon: <Users size={18} />,        adminOnly: true  },
  { key: 'events',         label: 'Create Event',    icon: <PlusCircle size={18} />,    adminOnly: true  },
  { key: 'club-members',   label: 'Club Members',    icon: <Users size={18} />,         adminOnly: false },
  { key: 'manage-events',  label: 'Manage Events',   icon: <Settings size={18} />,      adminOnly: false  },
  { key: 'enrolled-events',label: 'Enrolled Events', icon: <CalendarCheck size={18} />, adminOnly: false },
];
...
const SidebarContent = ({ activeTab, setActiveTab, club_role, handleLogout, closeMobile }) => {
  ...
  const visibleItems = NAV_ITEMS.filter(
    item => !item.adminOnly || club_role === 'admin'
  );
        ...
        {visibleItems.map(({ key, label, icon }) => {
          ...
          return (
            <button
              ...
              `}
            >
              <span className={`transition-colors ${isActive ? 'text-slate-900' : 'text-slate-500 group-hover:text-[#39D353]'}`}>
                {icon}
              </span>
              {label}
              {isActive && (
                <span className="ml-auto w-2 h-2 rounded-full bg-slate-900/40" />
              )}
            </button>
          );
        })}
...
const AdminDashboard = () => {
  ...
  const enrolledEvents = events.filter(e => (e.joined_users || []).includes(decoded?.user_id));
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-rose-50 via-amber-50 to-purple-50 text-slate-900 antialiased">
      ...
      <div
        className={`
          fixed top-0 left-0 h-full w-64 bg-slate-900 shadow-2xl z-50 
      ...
      <main className="flex-1 h-screen overflow-y-auto bg-white/50 backdrop-blur-sm flex flex-col">
        ...
        {/* ── Mobile Top Bar with LEFT hamburger ─────────── */}
      ...
      {selectedClub && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xl flex items-center justify-center z-50 px-4">
          <div className="relative bg-white/95 backdrop-blur-3xl w-full max-w-md sm:max-w-xl lg:max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl p-6 sm:p-10 shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={() => handleJoin(selectedClub.id)} className="bg-[#39D353] hover:bg-[#2bb545] text-slate-900 font-semibold px-6 py-2 rounded-xl transition w-full sm:w-auto">Join Club</button>
              <button onClick={() => setSelectedClub(null)} className="bg-gray-200 hover:bg-gray-300 px-6 py-2 rounded-xl transition w-full sm:w-auto">Cancel</button>
            </div>
          </div>
        </div>
      )}
