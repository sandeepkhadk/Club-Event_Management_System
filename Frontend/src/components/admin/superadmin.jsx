...
export default function SuperAdminPage({ superAdminId }) {
  ...
  // 🔥 ONE-TIME LOAD - No token dependency
  const loadData = async () => {
    console.log('🔍 SuperAdminPage LOADING - Token:', !!token, 'Decoded:', decoded);
  ...
  try {
    ...
    // 🔥 FIX: Don't logout on 403 (Forbidden) - only 401 (Unauthorized)
    if (res.status === 401) {
    ...
    // 🔥 ADD 403 handling for superadmin
    if (res.status === 403) {
      console.log('🔍 403 - Superadmin permission issue, but staying logged in');
      setClubs([]); // Show empty list instead of logout
      return;
    }
    ...
    console.error("Failed to fetch clubs:", err);
    setClubs([]); // Show empty list on error
  } finally {
    setLoading(false);
  }
};
...
const fetchUsers = async () => {
  ...
  try {
    ...
    if (res.status === 403) {
      console.log('🔍 403 - User fetch permission issue');
      setUsers([]);
      return;
    }
  ...
  // ✅ SHARED SUBMIT FUNCTION (Create OR Edit)
  ...
  try {
    ...
    const endpoint = formMode === "edit" 
      ? `${CLUB_API}${formClubId}/update/`  // /clubs/5/update/
      : `${CLUB_API}create/`;               // /clubs/create/
  ...
  // ✅ RESET FORM (for both create + edit)
  const resetForm = () => {
    setFormMode("create");
    setFormClubId(null);
    setClubName("");
  ...
  // ✅ START EDIT - Populate form with club data
  const startEdit = (club) => {
    ...
    // ✅ Try to set admin from existing data
  ...
  const deleteClub = async (clubId) => {
    ...
    const userConfirmed = window.confirm(`🚀 High-Level Clearance Required: Permanently dissolve "${clubToDelete.club_name}"?`);
    ...
    try {
      ...
      console.log(`✨ ${clubToDelete.club_name} has been banished.`);
    } catch (err) {
      console.error("The void refused the club:", err);
      setClubs(originalClubs); 
      alert(`💥 System Backfire: ${err.message}. The club has reappeared!`);
    }
  };
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {clubs.map((club) => (
                ...
                >
                  ...
                  {/* ✅ Admin shows SELECTED user's name */}
